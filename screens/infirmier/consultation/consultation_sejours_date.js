...
export default function Consultation_Sejours_Date({ navigation }) {
  ...
  const filteredSejours = useMemo(() => {
    ...
    return sejours
      .filter((sejour) => {
        ...
        const sortie = getDatePart(sejour.dateSortie);
        if (!entree || !sortie) {
          return false;
        }
        return entree <= normalizedSelectedDate && normalizedSelectedDate <= sortie;
      })
    ...
    <View style={styles.container}>
      <View style={[styles.card, styles.tableCard]}>
        <View style={[styles.cardHeader, styles.headerRow]}>
          <Text style={styles.cardTitle}>{`Sejours effectifs le ${formatDate(selectedDate)}`}</Text>
              ...
              <ScrollView style={styles.tableBodyScroll} nestedScrollEnabled>
                {filteredSejours.length > 0 ? (
                  ...
                  <View style={styles.emptyRow}>
                    <Text style={styles.emptyText}>Aucun sejour trouve pour cette date.</Text>
                  </View>
                )}
              </ScrollView>
            </View>
