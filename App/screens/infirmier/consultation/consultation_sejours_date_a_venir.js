...
export default function Consultation_Sejours_Date_A_Venir({ navigation }) {
  ...
  const filteredSejours = useMemo(() => {
    ...
    return sejours
      .filter((sejour) => {
        const entree = getDatePart(sejour.dateEntree);
        return entree && entree >= normalizedReferenceDate;
      })
      .sort((a, b) => getDatePart(a.dateEntree).localeCompare(getDatePart(b.dateEntree)));
  }, [sejours, normalizedReferenceDate]);

  const onResetDate = () => {
    setReferenceDate(toIsoDate(new Date()));
  };
    ...
    <View style={styles.container}>
      <View style={[styles.card, styles.tableCard]}>
        <View style={[styles.cardHeader, styles.headerRow]}>
          <Text style={styles.cardTitle}>{`Sejours a venir (a partir du ${formatDate(referenceDate)})`}</Text>
        ...
        <View style={styles.cardBody}>
          <View style={styles.formRow}>
            <View style={styles.formBlock}>
              <Text style={styles.formLabel}>Date de reference</Text>
              <TextInput
                value={referenceDate}
            ...
            <View style={styles.formActions}>
              ...
              <Pressable
                ...
                onPress={onResetDate}
              >
                <Text style={styles.btnTextLight}>Reinitialiser</Text>
              </Pressable>
            </View>
          </View>
              ...
              <ScrollView style={styles.tableBodyScroll} nestedScrollEnabled>
                {filteredSejours.length > 0 ? (
                  ...
                  <View style={styles.emptyRow}>
                    <Text style={styles.emptyText}>Aucun sejour a venir a partir de cette date.</Text>
                  </View>
                )}
              </ScrollView>
            </View>
          ...
          {filteredSejours.length > 0 ? (
            <View style={[styles.alert, styles.alertInfo]}>
              <Text style={styles.alertInfoText}>{`${filteredSejours.length} sejour(s) a venir affiche(s) par ordre chronologique.`}</Text>
            </View>
          ) : null}
        </View>
      </View>
